...
void DataNullBitMapTuple::unmarshallDataNullBitMapTuple(std::vector<uint8_t> inputData){ 
    if(inputData.size()==0){
        return;
    }
    int16_t type=0;
...
std::vector<uint8_t> DataNullBitMapTuple::marshallDataNullBitMapTuple(std::vector<bool>bitMap,std::vector<allVars>data){
